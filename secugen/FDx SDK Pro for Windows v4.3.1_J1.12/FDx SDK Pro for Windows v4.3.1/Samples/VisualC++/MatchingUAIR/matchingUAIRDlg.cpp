...
void CMatchingUAIRDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_DEL_REG_BUTTON, m_DelRegBtn);
}
...
BEGIN_MESSAGE_MAP(CMatchingUAIRDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_DEL_REG_BUTTON, &CMatchingUAIRDlg::OnBtnDeleteReg)
END_MESSAGE_MAP()
...
void CMatchingUAIRDlg::OnInitbutton() 
{
	...
	if (ExistMinFiles()) {
		m_DelRegBtn.EnableWindow();
	}

	// clear thumbnails for registration
	OnBtnClearReg();
	// clear image of verification
	OnBtnClearVerify();
...
////////////////////////////////////////////////////////////
void CMatchingUAIRDlg::OnBtnClearVerify()
{
	memset(m_ImgBuf, 0xff, m_ImageHeight*m_ImageWidth);
...
void CMatchingUAIRDlg::OnIdentify()
{
	...
	if (hfile < 0) {
		...
		m_ResultEdit = _T("Enroll your fingerprints. Not able to find .min files.");
		UpdateData(false);
		return;
	}
...
BOOL CMatchingUAIRDlg::ExistMinFiles()
{
	struct _finddata_t ffblk;
	intptr_t hfile;
...
void CMatchingUAIRDlg::OnBtnDeleteReg()
{
	int resp = MessageBox(_T("Deleting all .min files. Are you sure?"), _T("Delete Regsitration"), MB_YESNO | MB_ICONINFORMATION);
	if (resp == IDYES) {
		struct _finddata_t ffblk;
		intptr_t hfile;
		...
		hfile = _findfirst("*.min", &ffblk);
		if (hfile < 0) {
			m_ResultEdit = _T("Cannot find .min files");
		} else {
			char min_file[MAX_PATH];
			do {
				memset(min_file, 0, MAX_PATH);
				strcpy(min_file, ffblk.name);
				remove(min_file);
			} while (_findnext(hfile, &ffblk) == 0);
			m_ResultEdit = _T("All .min files have been deleted.");
		}
		_findclose(hfile);
	}

	if (!ExistMinFiles())
		m_DelRegBtn.EnableWindow(false); // disable
	UpdateData(false);
}
